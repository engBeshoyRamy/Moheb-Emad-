...
const SOCIALS = [
  {
    name: 'YouTube', href: 'https://www.youtube.com/@mohebemad', color: '#FF0000',
    icon: (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden><path d="M23.5 6.5s-.3-2-1.2-2.8c-1.1-1.2-2.4-1.2-3-1.3C16.8 2.2 12 2.2 12 2.2s-4.8 0-7.3.2c-.6.1-1.9.1-3 1.3C.8 4.5.5 6.5.5 6.5S.2 8.8.2 11.1v2.2c0 2.3.3 4.6.3 4.6s.3 2 1.2 2.8c1.1 1.2 2.6 1.1 3.3 1.2C7.2 22 12 22 12 22s4.8 0 7.3-.3c.6-.1 1.9-.1 3-1.3.9-.8 1.2-2.8 1.2-2.8s.3-2.3.3-4.6v-2.2c0-2.3-.3-4.6-.3-4.6zM9.7 15.5V8.4l8.1 3.6-8.1 3.5z" /></svg>),
  },
  {
    name: 'Facebook', href: 'https://www.facebook.com/mohebtadroos/', color: '#1877F2',
    icon: (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden><path d="M24 12.07C24 5.41 18.63 0 12 0S0 5.41 0 12.07C0 18.1 4.39 23.1 10.13 24v-8.44H7.08v-3.49h3.04V9.41c0-3.02 1.8-4.7 4.54-4.7 1.31 0 2.68.24 2.68.24v2.97h-1.51c-1.49 0-1.95.93-1.95 1.88v2.27h3.32l-.53 3.49h-2.79V24C19.62 23.1 24 18.1 24 12.07z" /></svg>),
  },
  {
    name: 'Instagram', href: 'https://www.instagram.com/e.tadroos', color: '#E1306C',
  ...
  return (
    <div aria-hidden style={{ display: 'flex', alignItems: 'center', gap: 3, height: 40, opacity: active ? 1 : 0.25, transition: 'opacity 0.4s' }}>
      {heights.map((h, i) => (
        <motion.div
          key={i}
          style={{ width: 2, borderRadius: 1, background: active ? `rgba(32,0,234,${0.5 + h * 0.5})` : 'rgba(32,0,234,0.4)', transformOrigin: 'center' }}
...
function FloatingField({ id, label, type = 'text', multiline = false, value, onChange, onBlur, error, required, rows = 5 }) {
  ...
  const sharedStyle = {
    ...
    border: error ? '1px solid rgba(255,80,80,0.6)' : focused ? '1px solid rgba(32,0,234,0.7)' : '1px solid var(--border)',
    borderRadius: 6,
    padding: multiline ? '28px 18px 14px' : '26px 18px 10px',
    color: 'var(--white)',
    fontFamily: 'var(--ff-body)', fontSize: '0.92rem',
    outline: 'none', boxSizing: 'border-box',
    transition: 'border-color 0.25s, background 0.25s, box-shadow 0.25s',
    boxShadow: focused ? (error ? '0 0 0 3px rgba(255,80,80,0.1)' : '0 0 0 3px rgba(32,0,234,0.1), 0 0 20px rgba(32,0,234,0.06)') : 'none',
  ...
  const labelStyle = {
    ...
    color: error ? 'rgba(255,100,100,0.9)' : lifted ? (focused ? 'rgba(32,0,234,0.9)' : 'var(--lav)') : 'var(--gray)',
    pointerEvents: 'none',
    ...
    >
      ...
      >
        <svg width="28" height="28" viewBox="0 0 28 28" fill="none" aria-hidden>
          <motion.path d="M6 14L11 19L22 8" stroke="rgba(34,197,94,0.9)" strokeWidth="2.2"
...
function validateForm({ name, email, subject, message }) {
  ...
  else if (message.trim().length < 20) errs.message = 'Please add more detail (min 20 chars)'
  return errs
}
        ...
        >
          ...
          >
            <div style={{
              ...
              borderRadius: 8, padding: 'clamp(28px, 4vw, 48px)',
              ...
              <motion.a
                ...
                style={{
                  ...
                  color: 'var(--lav)', letterSpacing: '0.1em', marginBottom: 28, textDecoration: 'none',
                }}
                whileHover={{ x: 4 }}
              ...
              <p style={{
                ...
                letterSpacing: '0.16em', color: 'var(--gray)', opacity: 0.5,
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <span style={{ fontFamily: 'var(--ff-mono)', fontSize: '0.56rem', letterSpacing: '0.28em', color: 'var(--blue)', textTransform: 'uppercase' }}>
                    New Transmission
                  </span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontFamily: 'var(--ff-mono)', fontSize: '0.52rem', letterSpacing: '0.14em', color: 'rgba(34,197,94,0.7)', textTransform: 'uppercase' }}>
                      Sends via WhatsApp
                    </span>
                  </div>
                </div>
              ...
              <AnimatePresence mode="wait">
                {status === 'sending' ? (
                  ...
                  >
                    ...
                    <div style={{
                      fontFamily: 'var(--ff-mono)', fontSize: '0.52rem', letterSpacing: '0.1em',
                      color: fields.message.length >= 20 ? 'rgba(34,197,94,0.6)' : 'var(--gray)',
                      textAlign: 'right', marginTop: -12, marginBottom: 24, transition: 'color 0.3s', opacity: 0.7,
                    }}>
                    ...
                    <p style={{
                      fontFamily: 'var(--ff-mono)', fontSize: '0.5rem', letterSpacing: '0.12em',
